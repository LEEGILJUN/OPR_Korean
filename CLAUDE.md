...
## 긴 산출물이 중간에 끊기지 않게 하는 규칙

독서 학습지 유형은 60~70문항에 참고 자료 기준 5만 자가 넘는다. 여기서 조기 종료를
일으킨 원인은 출력 한도가 아니라 **프롬프트 자신의 두 가지 결함**이었다. 둘 다 되살리기 쉬우니
아래를 지켜라.

**1. 형식별 개수를 정하는 유형에 "총 N개의 문항을 작성하라"를 붙이지 마라.**
`OutputType.count_applies_to`가 비어 있지 않으면 문항 수 설정은 그 형식(예: 객관식)에만
적용된다. 전체 문항 수로 쓰면 산출물 구성이 요구하는 70문항과 정면으로 부딪히고,
하필 프롬프트의 **마지막 문장**이라 모델이 그쪽을 따른다. 결과는 15문항짜리 학습지다.
잘린 것처럼 보이지만 실제로는 지시대로 만들고 정상 종료한 것이다.

**2. "분량이 많으면 멈추고 물어보라"류의 지시를 넣지 마라.**
멈추는 비용은 한 줄, 계속 쓰는 비용은 20문항이다. 탈출구를 주면 모델은 항상 그쪽을 고른다.
잘림을 막으려고 넣은 문구가 오히려 조기 종료를 만든다. 대신 "멈추지도 묻지도 마라,
문장 중간에서 잘려도 실패가 아니다"로 못 박는다.

재개는 마커가 아니라 **문항 번호**로 한다(`빈칸 12`, `OX 7`). 마커는 출력 맨 끝에 오는데
하드 절단은 바로 그 끝을 자르므로, 정작 필요할 때 출력되지 않는다. 번호는 본문 안에 있어
잘려도 남고 학습지에 그대로 인쇄돼도 자연스럽다.

같은 이유로 완료 마커도 쓰지 않는다. 모델은 자기가 방금 쓴 분량을 되짚어 세지 못해
선언한 숫자를 그대로 복사하며, 틀리는 경우가 정확히 그 마커가 잡아야 할 실패와 겹친다.

`tools/smoke_test.py`의 `check_long_form_protocol()`이 이 두 결함의 재발을 막는다.
