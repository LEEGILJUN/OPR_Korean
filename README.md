...
- 보조 모듈 지원
  ...
  - 난이도 미세조정 포함
- 평가 맥락 전환
  - 수능·모의고사: 난이도를 1등급 ~ 9등급으로
  - 중·고 내신: 난이도를 기초 ~ 최고난도로, 교과서·학년·단원 연계 입력
- 산출물 유형 선택 (유형에 따라 프롬프트가 통째로 바뀜, 출제 영역에 맞는 유형만 표시)
  - 문항 세트 (기본)
  - 문학 학습지 전체 — 단원 개념 정리 + 작품 해제 + 문항 + 정답·해설
  - 문학 작품 해제만 — 갈래·성격·주제·특징·시어 의미표 등, 문항 없음
  - 문법 개념 정리표 — 개념 체계표 + 예문 + 확인 문항
  - 미니 모의고사 — 수능형 지문 세트
  - 독서 학습지 전체 — 지문 분석(개요·도표·단계·전개 방식·보완점) + 서술형 문답 + 빈칸·OX·객관식
  - 독서 지문 분석 — 문항 없이 분석만
  - 독서 확인 문제 — 빈칸·OX·객관식만
- 난이도는 평가 맥락에 따라 다른 축을 사용
  - 등급별 목표 수준과 지침은 `config/difficulty_profiles.json`에서 관리
- 출제 옵션
  - 문항 형식: 객관식 5·4·3지선다 / 단답형 / 서술형 / 혼합 / 빈칸 채우기 / OX 진위 판단
  - 출제 묶음: 지문 세트형(수능형) / 독립 문항형 / 혼합형
  - 배점 구조: 수능형 2점·3점 혼합 / 균등 배점 / 고난도 3점 중심
  - 해설 배치: 문항 바로 뒤에 / 맨 뒤에 모아서
  - 여러 형식을 담을 때는 형식마다 문항 수를 따로 지정 (0으로 두면 해당 형식 제외)
    - 독서 학습지: 서술형 / 빈칸 / OX / 객관식
    - 문항 형식을 '혼합'으로 고른 경우: 객관식 / 단답형 / 서술형
- 자주 쓰는 프리셋 지원
  - 독서 기본형
  - 독서 고난도형
  - 현대시 기본형
...
2. 화면 아래 `검증 창 열기`를 누릅니다. (Ctrl+E)
3. 받은 문항을 그대로 붙여 넣고 검증 방식을 고른 뒤 `검증 프롬프트 생성`을 누릅니다.
